...
For eg. -> ls -alht /etc             # shows list of all files (including .hidden file) in long listing and human readable sizes which is sorted by modification time (newest first)
...
Quick Revision
•	Linux file system starts at / called root — everything branches from here
•	pwd tells you where you are, ls shows what is there, cd moves you
•	Linux is case sensitive — File.txt and file.txt are different files
•	Hidden files start with a dot — use ls -a to see them
•	man <command> opens the manual for any command — use it constantly
