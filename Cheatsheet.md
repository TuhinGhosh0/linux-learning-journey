...
Every command learned in this repository — in one place. Format: command — what it does — example
...
useradd	Create new user	sudo useradd -m -s /bin/bash john
passwd	Set user password	sudo passwd john
usermod	Modify user	sudo usermod -aG developers john
...
chown	Change owner	sudo chown john:dev file.txt
ls -l	View permissions	ls -la

Permission numbers: r=4, w=2, x=1 — add for combinations
